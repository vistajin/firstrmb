	...
	<div class="table-responsive">
		<table class="table table-striped table-hover">
			...
			<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
				...
				<td width="250px">
<!-- 					<button class="btn btn-info" onclick="copyFriendLink(<?php echo ($vo["uid"]); ?>)" title="复制"> -->
<!-- 						<span class="glyphicon glyphicon-duplicate"></span> -->
<!-- 					</button> -->
					<button class="btn btn-info" onclick="editFriendLinkInput(<?php echo ($vo["uid"]); ?>)" title="编辑">
						<span class="glyphicon glyphicon-pencil"></span>
					</button>
					<button class="btn btn-danger" data-toggle="modal" data-target="#myModal" 
						onclick="deleteFriendLink(<?php echo ($vo["uid"]); ?>)" title="删除">
						<span class="glyphicon glyphicon-remove"></span>
					</button>
					<!-- 只有显示的才有上下排序按钮 -->
					<?php if($vo['isshow']): ?><button class="btn btn-success" onclick="upFriendLink(<?php echo ($vo["uid"]); ?>)" title="上移">
							<span class="glyphicon glyphicon-arrow-up"></span>
						</button>
						<button class="btn btn-success" onclick="downFriendLink(<?php echo ($vo["uid"]); ?>)" title="下移">
...
$("#addbutton").click(function() {
    submitForm("/firstrmb/index.php/Admin/HomeMaint/addFriendLinkInput");
});

function editFriendLinkInput(uid) {
    $("#uid").val(uid);
    submitForm("/firstrmb/index.php/Admin/HomeMaint/editFriendLinkInput");
}

function upFriendLink(uid) {
    $("#uid").val(uid);
    submitForm("/firstrmb/index.php/Admin/HomeMaint/upFriendLink");
}
function downFriendLink(uid) {
    $("#uid").val(uid);
    submitForm("/firstrmb/index.php/Admin/HomeMaint/downFriendLink");
}
...
function doDeleteFriendLink() {
    submitForm("/firstrmb/index.php/Admin/HomeMaint/deleteFriendLink")
}
</script>
</html>